...
function format (str) {
  str = str.trim()
  let res = match([
    str.match(/^(0|255|25[0-4]|2[0-4]\d|1\d\d|0?\d?\d),(0|255|25[0-4]|2[0-4]\d|1\d\d|0?\d?\d),(0|255|25[0-4]|2[0-4]\d|1\d\d|0?\d?\d)$/),
    m => { return { r: m[1], g: m[2], b: m[3] } },
    str.match(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i),
    m => { return { r: parseInt(m[1], 16), g: parseInt(m[2], 16), b: parseInt(m[3], 16) } }
  ])
  if (!res) {
    let num = parseInt(str)
    if (!isNaN(num) && num >= 0 && num <= 16777215) {
      res = { r: (num >> 16) & 0xFF, g: (num >> 8) & 0xFF, b: num & 0xFF }
    } else return null
  }
  res.hex = rgbToHex(res.r, res.g, res.b)
  res.name = COLORS[closest(COLORS, res.hex)]
  res.int = ((res.r & 0x0ff) << 16) | ((res.g & 0x0ff) << 8) | (res.b & 0x0ff)
  return res
}
...
function rgbToHex (...args) {
  ...
  for (let i = 0; i < args.length; i++) {
    let hex = Number(args[i]).toString(16)
